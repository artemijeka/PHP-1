...
////////////////////////////////////////////////////////////////////////////////////////////////////
/***************************** ТИПЫ ПЕРЕМЕННЫХ ИЛИ РЕЗУЛЬТАТ ФУНКЦИИ: *****************************/
// ПРИМЕР:
(int) $var = 5;// указывается перед переменной
function example($number):int {
	// для функции после объявления фунцкии и ":" уквзывается тип возвращаемого результат
	return $number;
}
/* Имя класса */
...
// FOREACH
foreach ($array as $key => &$value) {}/* Для того, чтобы напрямую изменять элементы массива внутри
цикла, переменной $value должен предшествовать знак &.
 */
...
array_slice($array, 0, 5, true);/* array_slice() возвращает последовательность элементов
массива array, определённую параметрами offset и length. Если параметр offset положителен,
последовательность начнётся на расстоянии offset от начала array. Если offset отрицателен,
последовательность начнётся на расстоянии offset от конца array. Если в эту функцию передан
положительный параметр length, последовательность будет включать length элементов. Если в эту
функцию передан отрицательный параметр length, в последовательность войдут все элементы
исходного массива, начиная с позиции offset и заканчивая позицией, отстоящей на length элементов
от конца array. Если этот параметр будет опущен, в последовательность войдут все элементы
исходного массива array, начиная с позиции offset. Обратите внимание, что array_slice()
сбрасывает ключи массива. Начиная с PHP 5.0.2 вы можете переопределить это поведение, установив
 */
...
// сначала указывается корень пространства имен \ потом имя пространства Tiny, затем снова разделитель \ и далее имя класса Fruit, после имя статичного метода через :: munch();
...
// вместо \Tiny\Eating\Fruit::munch();
Fruit::munch("orange");
// вместо \Tiny\Fruit::munch();
